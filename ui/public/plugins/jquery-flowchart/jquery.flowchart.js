...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        createLink: function (linkId, linkDataOriginal) {
            ...
            var resourceLink = linkData.fromConnector.match(/resource.*/);
            ...
            this._autoCreateSubConnector(linkData.fromOperator, linkData.fromConnector, resourceLink ? 'resources' : 'outputs', fromSubConnector);
        ...
        getOperatorCompleteData: function (operatorData) {
            ...
            // adding functionality by @max-ionov
            for (var connectorId_res in infos.resources) {
                if (infos.resources.hasOwnProperty(connectorId_res)) {
                    if (infos.resources[connectorId_res] == null) {
                        delete infos.resources[connectorId_res];
                    }
                }
            }
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            // adding functionality by @max-ionov
            var $operator_resources = $('<div class="flowchart-operator-resources"></div>');
            $operator_resources.appendTo($operator_inputs_outputs);
            ...
            // added by @max-ionov, resources
            for (var key_r in infos.resources) {
                if (infos.resources.hasOwnProperty(key_r)) {
                    addConnector(key_r, infos.resources[key_r], $operator_resources, 'resources');
                }
            }
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
            ...
            var connectorType = $operator_connector_set.data('connector_type');
        ...
        _connectorClicked: function (operator, connector, subConnector, connectorCategory) {
            if (connectorCategory == 'outputs' || connectorCategory == 'resources') {
                var d = new Date();
        ...
        _deleteLink: function (linkId, forced) {
            ...
            this._cleanMultipleConnectors(fromOperator, fromConnector, 'from', fromConnector.match(/resource.*/) ? 'resources': 'outputs');
            this._cleanMultipleConnectors(toOperator, toConnector, 'to', 'inputs');

            this.callbackEvent('afterChange', ['link_delete']);
        },

        _cleanMultipleConnectors: function (operator, connector, linkFromTo, operatorType) {
            if (!this.data.operators[operator].internal.properties[operatorType][connector].multiple) {
                return;
            }
        ...
        setOperatorOptions: function (operatorId, options) {
            if (typeof this.data.operators[operatorId].properties == 'undefined') {
                this.data.operators[operatorId].properties = {};
            }
            this.data.operators[operatorId].properties.options = options;
            this._refreshInternalProperties(this.data.operators[operatorId]);
            this.callbackEvent('afterChange', ['operator_options_change']);
        },
